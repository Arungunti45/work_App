...
exports.hireApplicant = exports.changeApplicationStatus = exports.submitApplication = void 0;
 ...
 */
async function createAuditLog(action, actorUid, targetId, metadata = {}) {
 ...
 */
async function addStatusHistory(applicationId, status, changedBy, reason) {
...
exports.submitApplication = functions.https.onCall(async (data, context) => {
    ...
    await (0, notifications_1.sendPushToUser)(jobData.employerId, 'New application received', `A worker applied for: ${jobTitle}`, `/employer/applications`);
    return { success: true, applicationId: applicationRef.id };
});
exports.changeApplicationStatus = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (newStatus === 'SHORTLISTED')
            updateData.shortlistedAt = updateData.updatedAt;
        if (newStatus === 'REJECTED')
            updateData.rejectedAt = updateData.updatedAt;
        if (newStatus === 'WITHDRAWN')
            updateData.withdrawnAt = updateData.updatedAt;
        ...
        if (notif) {
            ...
            await (0, notifications_1.sendPushToUser)(notif.recipientId, notif.title, notif.body);
        }
        return { success: true };
    });
});
exports.hireApplicant = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (app) {
            ...
            await (0, notifications_1.sendPushToUser)(app.workerId, 'Congratulations! You are hired!', `You have been hired for: ${jobTitle}`);
        }
        return { success: true };
    });
});
