...
exports.unblockUser = exports.blockUser = exports.createReport = exports.submitVerification = void 0;
...
exports.submitVerification = functions.https.onCall(async (data, context) => {
    ...
    if (!existingPending.empty) {
        throw new functions.https.HttpsError('already-exists', 'A verification request is already pending for this type. Please wait for review.');
    }
    ...
    const profileCollection = userRole === 'WORKER' ? 'workerProfiles'
...
exports.createReport = functions.https.onCall(async (data, context) => {
    ...
    const uid = context.auth.uid;
    const { targetType, targetId, category, description, conversationId, messageId } = data;
    ...
    if (!duplicateCheck.empty) {
        throw new functions.https.HttpsError('already-exists', 'You have already submitted a similar report recently. Our team is reviewing it.');
    }
