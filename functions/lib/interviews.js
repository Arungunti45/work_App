...
exports.updateInterviewStatus = exports.scheduleInterview = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
const notifications_1 = require("./notifications");
const db = admin.firestore();
exports.scheduleInterview = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    await (0, notifications_1.sendPushToUser)(appData.workerId, 'Interview scheduled', `Your interview for ${jobTitle} is on ${schedDate}.`);
...
exports.updateInterviewStatus = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    ...
    if (notifData) {
        ...
        await (0, notifications_1.sendPushToUser)(interviewData.workerId, notifData.title, notifData.body);
    }
    return { success: true };
});
